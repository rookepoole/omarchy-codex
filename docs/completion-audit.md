...
| --- | --- | --- |
| Correct graphical product boundary | Official Linux ChatGPT package `26.901.41600`; bundled Codex core; `Terminal=false`; no `/usr/bin/codex` replacement | Proven |
...
| Optional Quattro panel | Marketplace manifest, official plugin-validator pass, read-only status boundary, and physical Omarchy verification | Proven; add/enable, panel display, version reporting, and graphical launch confirmed |
...
| Update behavior | Panel opens reviewed releases without downloading or executing source; installer accepts only an existing checkout or checksummed bundle | Proven in source for 0.1.10; next physical install pending |
...
| Versioned source and installer artifact | Git tag/release plus checksumed payload-free installer bundle; OpenAI payload remains downloaded and verified locally | Proven for v0.1.9; v0.1.10 pending publication |
